...
fn semver_regex() -> &'static Regex {
    ...
    SEMVER_RE.get_or_init(|| Regex::new(r"^\d+\.\d+\.\d+$").expect("SEMVER_RE regex must compile"))
}

pub fn validate_version(version: &str) -> Result<(), String> {
    if !semver_regex().is_match(version) {
        Err(format!(
            "Invalid version '{}'. Version numbers should follow semantic versioning.",
            version
        ))
    } else {
        Ok(())
...
pub async fn compare_worker_version(
    worker_name: &str,
    version: &str,
    client: &DockerClient,
    ...
    match latest_version {
        Some(latest) => compare_versions(version, &latest),
